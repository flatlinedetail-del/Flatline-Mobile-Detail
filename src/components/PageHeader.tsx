  ...
  return (
    <div className={cn("flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10 pb-2 border-b border-white/5", className)}>
      <div className="space-y-3">
        <h1 className="text-4xl sm:text-5xl md:text-6xl font-black tracking-tighter uppercase leading-tight text-white font-heading">
          {accentWord ? (
            <>
              {parts[0]}
        ...
        {subtitle && (
          <p className="text-[11px] md:text-xs font-black text-white/50 uppercase tracking-[0.25em] flex items-center gap-2.5 ml-1">
            <span className="flex h-1.5 w-1.5 rounded-full bg-primary shadow-[0_0_8px_rgba(229,57,53,0.5)]"></span>
            {subtitle}
          </p>
        )}
      </div>
